function makeListConstellations ({ constellationsDb }) {
  return async function listConstellations ({ consName, type } = {}) {
    ...
    const constellations = await constellationsDb.find({ type, consName })
    if (!constellations) {
      throw new Error('服务器端获取数据失败，请查看服务器打印数据!')
    }
...
module.exports = makeListConstellations